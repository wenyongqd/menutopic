...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const ensureUserProfile = async (userId: string) => {
    try {
      ...
      if (!data) {
        ...
        if (insertError) {
          console.error(
            "AuthProvider - Error creating user profile:",
            insertError
          );
          return;
        }
      ...
      console.error(
        "AuthProvider - Failed to ensure user profile exists:",
        error
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const {
      ...
      console.log(
        ...
        event,
        session ? `for user ${session.user.id}` : "no session"
      );
      ...
      if (event === "SIGNED_IN") {
        ...
        console.log(
          "AuthProvider - Other auth event, checking if state needs update"
        );
        ...
        if (needsUpdate) {
          ...
          console.log(
            "AuthProvider - Auth state unchanged, preserving current state"
          );
        }
  ...
  const refreshData = useCallback(() => {
    ...
    if (
      ...
      console.log(
        "AuthProvider - Just logged in state detected, using special refresh flow"
      );
    ...
    console.log(
      "AuthProvider - Current user before refresh:",
      currentUser?.id || "null"
    );
    ...
    new Promise<void>(async (resolve) => {
      try {
        ...
        if (error) {
          ...
          // 出错时恢复原始状态
          console.log(
            "AuthProvider - Error during refresh, preserving original state"
          );
          resolve();
          return;
        }
        ...
        if (
          ...
          console.log(
            "AuthProvider - User state changed during refresh, updating"
          );
          setSession(newSession);
          setUser(newUser);
        } else {
          console.log(
            "AuthProvider - User state unchanged during refresh, preserving current state"
          );
          // 确保状态保持一致
          if (currentUser && !user) {
        ...
        if (newUser && !user) {
          console.log(
            "AuthProvider - Inconsistent state detected, fixing user state"
          );
          setUser(newUser);
        }
        ...
        if (currentUser && !user) {
          console.log(
            "AuthProvider - Error during refresh, restoring user state"
          );
          setUser(currentUser);
        }
        resolve();
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "AuthProvider - Dashboard visit recorded, preserving authentication state"
      );
      ...
      if (typeof window !== "undefined") {
        ...
        if (justLoggedIn) {
          console.log(
            "AuthProvider - Just logged in state detected, preserving session"
          );
          // 不执行任何可能导致状态重置的操作
        }
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "AuthProvider - Protected page loaded without user, checking session"
      );
      getSession();
    }
  ...
  const signUp = async (email: string, password: string) => {
    ...
    const redirectPath =
      currentUrl.searchParams.get("redirect") || "/dashboard";
  ...
  const signOut = async () => {
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        // 清除 cookie
        document.cookie =
          "just_logged_in=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
      }
